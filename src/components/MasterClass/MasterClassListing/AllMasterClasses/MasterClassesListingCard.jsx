...
export default function MasterClassesListingCard({ item }) {
  return (
    <div className="custom-card upcoming-class-card rounded-12 border-w-2 border-color-1 h-100">
      <div className="custom-card-body p-0">
        <div className="img-box">
          {(item?.custom_url && (
            <Link href={`${item?.custom_url}`} target="_blank" rel="nofollow">
              {(item?.class_short_thumbnail && (
                <Image
                  ...
                  width={429}
                  height={216}
                  className="img-fluid w-100 rounded-10"
                  alt="Image"
                />
              )) || (
                <Image
                  src={process.env.IMG_PATH + 'images/master-class/masterclass-listing-banner-img-1.webp'}
                  width={429}
                  height={216}
                  className="img-fluid w-100 rounded-10"
                  alt="Image"
            ...
            ((item?.class_status == 'registration-open' || item?.class_status == 'join-now') && (
              <Link
                href={
                  (item?.class_status == 'registration-open' && `/events/${item?.slug_url}`) ||
                  (item?.class_status == 'join-now' && `/events/${item?.slug_url}`) ||
                  ''
                }
              >
                {(item?.class_short_thumbnail && (
        ...
        <div className="p-lg-24 p-20 pb-0">
          {(item?.custom_url && (
            <Link href={`${item?.custom_url}`} target="_blank" rel="nofollow">
            ...
            ((item?.class_status == 'registration-open' || item?.class_status == 'join-now') && (
              <Link
                ...
                className={
                  ...
                  (item?.class_status == 'join-now' && 'cursor-pointer') ||
                  'cursor-default'
                }
              >
                <h3 className="fs-18 lh-27 fw-600 text-color-1 mb-3">{item?.class_title}</h3>
          ...
          {/* <div className="d-flex align-items-center mb-0 user-review-list">
            <div className="d-lg-flex d-none align-items-center enrolled-users max-w-100">
              {item?.userImages?.map((user, childIndex) => {
                return (
                  <div
                    className="rounded-circle min-w-36 h-size-36 border-w border-color-1 justify-content center card-shadow-2 d-flex align-items-center enrolled-user"
                    key={childIndex}
                  >
                    <Image src={user?.image} className="img-fluid" alt="Image" width={36} height={36} />
                  </div>
                );
              })}
            </div>
            <p className="m-0 text-color-15 lh-21 fs-14">
              <span className="fw-600 text-color-16">{formatNumberCount(item?.class_enrolled_count)}</span> Enrolled
            </p>
          </div> */}
        </div>
      </div>
      <div className="new-card-footer border-0 p-lg-24 p-20 ">
        {(item?.class_status == 'registration-open' && (
        ...
        {(item?.class_status == 'expired' && item?.class_enrolled_count && (
          <span className="fs-15 text-color-7 fw-500">
            <b className="fw-600">{numberFormat(Number(item?.class_enrolled_count))}</b> people have participated
          </span>
        )) ||
          ''}
        {/* {(item?.custom_url && (
