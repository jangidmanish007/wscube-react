...
function DocBackModal(props) {
  const { setShowBackModal, setBackDocument, backDocument, setBackPreview, setIsBIdChanged, userStatus } = props;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${String(backDocument?.lastModified)}back-image.jpeg`, { type: 'image/jpeg' });

      // Set the File object to backDocument and update the preview
      setBackDocument(file);
      setBackPreview(croppedImage);
      if (userStatus?.is_id_proof_approved === false) {
    ...
    <>
      <Modal show={true} centered backdrop="static" keyboard={false} size="lg" backdropClassName="ws-modal-backdrop">
        <Modal.Body className="p-16">
          <div className="">
            <h4 className="fs-18 fw-600 text-color-1 lh-30">Upload Back Picture</h4>
            <Image
              ...
              <>
                <div className="mb-3 w-100">
                  <div className="d-flex align-items-center w-100 border rounded-8">
                    ...
                    {(backDocument && (
                      <span className="ml-12 fs-14 fw-400 lh-24 text-color-7 line-clamp-1">{backDocument?.name}</span>
                  ...
                  <small className="fs-12 mt-1 text-color-7">
