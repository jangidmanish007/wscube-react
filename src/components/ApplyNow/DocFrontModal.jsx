...
function DocFrontModal(props) {
  const { setShowFrontModal, setFrontDocument, frontDocument, setFrontPreview, setIsFIdChanged, userStatus } = props;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${String(frontDocument?.lastModified)}front-image.jpeg`, { type: 'image/jpeg' });

      // Set the File object to frontDocument and update the preview
      setFrontDocument(file);
      setFrontPreview(croppedImage);
      if (userStatus?.is_id_proof_approved === false) {
        setIsFIdChanged(true);
      }
    ...
    <>
      ...
      >
        <Modal.Body className="p-16">
          <div className="">
            <h4 className="fs-18 fw-600 text-color-1 lh-30">Upload Front Picture</h4>
            <Image
            ...
            {(imageSrc && (
              ...
              <div className="mb-3 w-100">
                <div className="d-flex align-items-center w-100 border rounded-8">
                  ...
                  {(frontDocument && (
                    <span className="ml-12 fs-14 fw-400 lh-24 text-color-7 line-clamp-1">{frontDocument?.name}</span>
