...
export default function CouresDetails({ details, params, stickyAdsData }) {
  ...
  const lmLeadCourse = details?.lm_lead_course;
  return (
    <>
      <HompageLead
        ...
        centerSlug={(params?.course && params?.slug) || ''}
      ...
      {/* {(params?.slug == 'data-analytics-course' && (
        <>
          <CourseFeeStructureDA
            fee={details?.fee}
            setShowLeadModal={setShowLeadModal}
            setLeadNote={setLeadNote}
            setDownloadCrs={setDownloadCrs}
